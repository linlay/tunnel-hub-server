...
Zenmind Tunnel 是一个自托管的 HTTP/WebSocket 反向隧道服务，目标是提供类似 Cloudflare Tunnel 的基础能力：本地 Agent 主动连接公网 Relay，公网请求按域名路由到内网服务。
...
1. 公网用户访问 `app.example.com`。
2. Relay 根据请求 Host 查询已启用路由，例如 `app.example.com -> http://127.0.0.1:3000`。
...
管理 API 前缀为 `/api/admin`，登录后使用 HttpOnly cookie `zenmind_admin` 鉴权。本地开发 CORS 允许 `localhost` 与 `127.0.0.1` 来源携带 cookie。
...
- `ADMIN_HOST` 应使用独立管理域名，不建议和业务路由域名混用。
