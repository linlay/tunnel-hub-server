...
func TestSQLiteConversationShareV2ClaimAndAttachmentLifecycle(t *testing.T) {
	...
	share, err := db.CreateConversationShareWithAttachments(ctx, "owner", "chat", 2, []byte(`{"version":2}`),
	...
	for i := 0; i < 8; i++ {
		...
		go func() {
			defer wg.Done()
			_, claimed, err := db.AccessPublicConversationShareV2(ctx, share.ID, now, nil, digest[:])
			if err == nil && claimed {
				mu.Lock()
				successes++
				mu.Unlock()
	...
	if _, _, err := db.AccessPublicConversationShareV2(ctx, share.ID, now, nil, digest[:]); !errors.Is(err, ErrNotFound) {
		t.Fatalf("second browser access=%v", err)
	}
	if _, claimed, err := db.AccessPublicConversationShareV2(ctx, share.ID, now.Add(time.Minute), digest[:], nil); err != nil || claimed {
